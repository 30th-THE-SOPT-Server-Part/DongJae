...
const createBlog = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const data: PostReponseDto = await BlogService.createBlog(blogCreateDto);
 ...
 *  @access Public
 */
const findBlog = async(req: Request, res: Response): Promise<void> => {
    const { blogId } = req.params;

    try{
        const data = await BlogService.findBlog(blogId);
